...
class CplBpmController extends Controller
{
    /**
...
{
    $title = 'Target Pemenuhan capaian Pembelajaran CPL BPM';
    ...
    $tahunAkademik = CplBpm::distinct('tahun_akademik')->pluck('tahun_akademik')->sort();
    $programStudi = CplBpm::distinct('program_studi')->pluck('program_studi')->sort();

    return view('admin.cplbpm.index', compact('title', 'targets', 'tahunAkademik', 'programStudi'));
}
    ...
    {
        $title = 'Tambah Target CPL BPM';
        return view('admin.cplbpm.create', compact('title'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('cpl-bpm')->with('success', 'Data target berhasil ditambahkan!');
    }

    /**
     * Show the form for editing the specified resource.
     */
    public function edit(CplBpm $cplBpm)
    {
        $title = 'Edit Target Rektorat';
        return view('admin.cplbpm.edit', compact('title', 'cplBpm'));
    }

    /**
     * Update the specified resource in storage.
     */
    public function update(Request $request, CplBpm $cplBpm)
    {
        // Validasi input
        $request->validate([
            'tahun_akademik' => 'required|string|max:255|regex:/^\d{4}\/\d{4}$/',
        ...
        // Redirect dengan pesan sukses
        return redirect()->route('cpl-bpm')->with('success', 'Data target berhasil diperbarui!');
    ...
    {
        try {
            return Excel::download(new CplBpmExport, 'cpl_bpm_' . date('Y-m-d_H-i-s') . '.xlsx');
        } catch (\Exception $e) {
    ...
    public function exportPdf()
    {
        try {
            ...
            $pdf = Pdf::loadView('cplBpm.pdf', compact('targets'));

            return $pdf->download('cpl_bpm_' . date('Y-m-d_H-i-s') . '.pdf');
        } catch (\Exception $e) {
     ...
     */
    public function destroy(CplBpm $cplBpm)
{
    try {
        $cplBpm->delete();
        return redirect()->route('cpl-bpm')->with('success', 'Data berhasil dihapus!');
    } catch (\Exception $e) {
        Log::error('Error deleting cpl-bpm: ' . $e->getMessage());
        return redirect()->route('cpl-bpm')->with('error', 'Gagal menghapus data: ' . $e->getMessage());
    }
}
}
