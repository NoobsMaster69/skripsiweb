...
class CpProdiController extends Controller
{
    /**
    ...
    {
        $title = 'Target Pemenuhan capaian Pembelajaran CPL Prodi';

        // Query dasar
        $query = CplProdi::query();
        ...
        $tahunAkademik = CplProdi::distinct('tahun_akademik')->pluck('tahun_akademik')->sort();
        $programStudi = CplProdi::distinct('program_studi')->pluck('program_studi')->sort();

        return view('prodi.fti.capaian-pembelajaran.index', compact('title', 'targets', 'tahunAkademik', 'programStudi'));
    }
    ...
    {
        $title = 'Tambah Target CPL Prodi';
        return view('prodi.fti.capaian-pembelajaran.create', compact('title'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('cp-prodi-fti')->with('success', 'Data target berhasil ditambahkan!');
    }
    ...
    {
        $title = 'Edit Target CPL Prodil';
        return view('prodi.fti.capaian-pembelajaran.edit', compact('title', 'cplProdi'));
    }
    ...
    public function update(Request $request, CplProdi $cplProdi)
    {
        ...
        // Redirect dengan pesan sukses
        return redirect()->route('cp-prodi-fti')->with('success', 'Data target berhasil diperbarui!');
    }
    ...
    {
        try {
            return Excel::download(new CplBpmExport, 'cpl_prodi' . date('Y-m-d_H-i-s') . '.xlsx');
        } catch (\Exception $e) {
    ...
    {
        try {
            $targets = CplProdi::all();

            $pdf = Pdf::loadView('cplProdi.pdf', compact('targets'));

            return $pdf->download('cpl_prodi' . date('Y-m-d_H-i-s') . '.pdf');
        } catch (\Exception $e) {
    ...
    public function destroy(CplProdi $cplProdi)
    {
        try {
            $cplProdi->delete();
            return redirect()->route('cpl_prodi')->with('success', 'Data berhasil dihapus!');
        } catch (\Exception $e) {
            Log::error('Error deleting cpl_prodi: ' . $e->getMessage());
            return redirect()->route('cpl_prodi')->with('error', 'Gagal menghapus data: ' . $e->getMessage());
        }
    }
}
