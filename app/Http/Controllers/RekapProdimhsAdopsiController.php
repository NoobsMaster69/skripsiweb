...
class RekapProdimhsAdopsiController extends Controller
{
    public function index(Request $request)
    {
        $title = 'Rekap Prodi "Karya Mahasiswa yang di Adopsi oleh Masyarakat"';
        ...
        if ($request->has('submit_filter')) {
            ...
            // ✅ Menampilkan semua data jika filter kosong
            $data = $query->orderByDesc('tanggal_perolehan')->get();
        } else {
            $data = collect(); // sebelum klik "Tampilkan Data", kosong
        }
        ...
        return view('prodi.fti.rekap.rekapMhsAdop', compact('title', 'data', 'request', 'listProdi', 'listTahun'));
    }
    ...
    public function exportPDF(Request $request)
    {
        ...
        $pdf = Pdf::loadView('prodi.fti.rekap.mhsadopsi_pdf', [
            'data' => $data,
    ...
    public function grafik(Request $request)
    {
        ...
        return view('prodi.fti.rekap.grafik.grafikMhsAdopsi', compact(
            'title',
            'grafikData',
            'listTahun',
            'listProdi',
