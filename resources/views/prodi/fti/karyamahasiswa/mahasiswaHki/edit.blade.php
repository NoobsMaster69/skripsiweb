...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('mahasiswa-hki-fti') }}">Mahasiswa yg dapat HKI</a></li>
            ...
            <div class="card shadow">
                <div class="card-header py-3 d-flex align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">
                        <i class="fas fa-pen mr-2"></i>Form Edit Data Karya Mahasiswa
                    </h6>
                </div>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('mahasiswa-hki-fti.update', $karyaMahasiswa->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')

                        <div class="form-group">
                            <label for="nm_mhs" class="font-weight-bold">Nama Lengkap Mahasiswa <span class="text-danger">*</span></label>
                            <input type="text" name="nm_mhs" id="nm_mhs" class="form-control @error('nm_mhs') is-invalid @enderror"
                                   value="{{ old('nm_mhs', $karyaMahasiswa->nm_mhs) }}" required>
                            @error('nm_mhs')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="nim" id="nim" class="form-control @error('nim') is-invalid @enderror"
                                   value="{{ old('nim', $karyaMahasiswa->nim) }}" required>
                            @error('nim')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="prodi" id="prodi" class="form-control @error('prodi') is-invalid @enderror"
                                   value="{{ old('prodi', $karyaMahasiswa->prodi) }}" required>
                            @error('prodi')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="fakultas" id="fakultas" class="form-control @error('fakultas') is-invalid @enderror" required>
                                ...
                                <option value="FTI" {{ old('fakultas', $karyaMahasiswa->fakultas) == 'FTI' ? 'selected' : '' }}>FTI</option>
                                <option value="FEB" {{ old('fakultas', $karyaMahasiswa->fakultas) == 'FEB' ? 'selected' : '' }}>FEB</option>
                            </select>
                            @error('fakultas')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="tahun" id="tahun" class="form-control @error('tahun') is-invalid @enderror"
                                   value="{{ old('tahun', $karyaMahasiswa->tahun) }}" required>
                            @error('tahun')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            <label for="judul_karya" class="font-weight-bold">Judul Karya <span class="text-danger">*</span></label>
                            <input type="text" name="judul_karya" id="judul_karya" class="form-control @error('judul_karya') is-invalid @enderror"
                                   value="{{ old('judul_karya', $karyaMahasiswa->judul_karya) }}" required>
                            @error('judul_karya')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="tanggal_perolehan" id="tanggal_perolehan" class="form-control @error('tanggal_perolehan') is-invalid @enderror"
                                   value="{{ old('tanggal_perolehan', optional($karyaMahasiswa->tanggal_perolehan)->format('Y-m-d')) }}" required>
                        ...
                        <div class="form-group">
                            ...
                            <small class="form-text text-muted">
                                Kosongkan jika tidak ingin mengubah file. Format: PDF, DOCX, JPG, JPEG (maks 2MB).
                            </small>
                            @if ($karyaMahasiswa->file_upload)
                                <p class="mt-2 mb-0 small">
                                    File saat ini:
                                    <a href="{{ asset('storage/'.$karyaMahasiswa->file_upload) }}" target="_blank">
                                        {{ basename($karyaMahasiswa->file_upload) }}
                                    </a>
                                </p>
                            @endif
                            @error('file_upload')
            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <p class="small text-muted">Isi nama lengkap sesuai data kampus.</p>
                    </div>
                    <div class="mb-3">
                        <h6 class="font-weight-bold text-primary">🆔 NIM Mahasiswa</h6>
                        <p class="small text-muted">Masukkan NIM resmi mahasiswa.</p>
                    </div>
                    <div class="mb-3">
                        <h6 class="font-weight-bold text-primary">🏫 Program Studi & Fakultas</h6>
                        <p class="small text-muted">Pastikan sesuai unit akademik mahasiswa.</p>
                    </div>
                    <div class="mb-3">
                        <h6 class="font-weight-bold text-primary">📚 Judul Karya</h6>
                        <p class="small text-muted">Judul lengkap karya yang didaftarkan HKI.</p>
                    </div>
                    <div class="mb-3">
                        <h6 class="font-weight-bold text-primary">📅 Tanggal Perolehan</h6>
                        <p class="small text-muted">Tanggal resmi saat mendapatkan HKI.</p>
                    </div>
                    <div class="alert alert-warning">
                        ...
                        <ul class="mb-0 mt-2 small">
                            ...
                            <li>Periksa ulang semua data sebelum simpan.</li>
                        </ul>
                    </div>
                </div>
            </div>
